(function () {
  ...
  const state = {
    ...
    config: {
      ...
      language: {
        ...
        lookupCharacterPolicy: IINATAN_LOOKUP_CHARACTER_POLICY.policies.japanese,
      },
  ...
  function controllerBindings() {
    return normalizeControllerBindings(state.config && state.config.controllerBindings);
  }

  function controllerActionForButton(button, context) {
    const selectedContext =
      context === "audioList" || context === "withPopup" || context === "noPopup"
        ? context
        : controllerContext();
  ...
  function normalizeAnkiConfig(config) {
    ...
    return {
      ...
      duplicateMode:
        String(raw.duplicateMode || "prevent") === "allow" ? "allow" : "prevent",
      duplicateScope:
        String(raw.duplicateScope || "deck") === "collection" ? "collection" : "deck",
  ...
  function audioSourceDisplayLabel(source, index) {
    ...
    if (/^(?:https?:\/\/)?(?:127\.0\.0\.1|localhost):5050(?:[/?#]|$)/i.test(url))
      return "Local audio";
    try {
      if (typeof URL === "function") {
  ...
  function audioLanguageCode() {
    ...
    return String(
      (lang && lang.id) || (state.config && state.config.lookupLanguage) || "ja",
    );
  }
  ...
  function audioAnkiSelectionForTerm(term, reading) {
    return state.audioAnkiSelections[audioTermReadingKey(term, reading)] || null;
  }
  function audioAnkiSelectionMatches(selection, sourceIndex, candidateIndex) {
    ...
    const selectedCandidateIndex =
      selection.candidateIndex === null ? null : Number(selection.candidateIndex);
  ...
  function setAudioAnkiSelection(
    ...
    const selection = {
      ...
      candidateIndex: candidateIndex === null ? null : Number(candidateIndex) || 0,
    };
  ...
  function parseAudioSourceListJson(value, sourceUrl) {
    ...
    if (!data || data.type !== "audioSourceList" || !Array.isArray(data.audioSources))
      return null;
    const urls = [];
  ...
  function requestAudioCandidatesFromPlugin(sourceUrl, term, reading, source) {
  ...
  async function resolveAudioCandidateUrls(source, term, reading) {
    ...
    const bridgeResult = await requestAudioCandidatesFromPlugin(
      sourceUrl,
      term,
      reading,
      source,
  ...
  function waitForAudioData(audio, timeoutMs) {
    return new Promise((resolve, reject) => {
      ...
      try {
        if (Number.isFinite(Number(audio.readyState)) && Number(audio.readyState) >= 2)
          finish(true);
  ...
  async function loadPlayableAudioCandidate(
    ...
    return {
      ...
      sourceIndex,
      candidateIndex,
      sourceName: candidate.name || audioSourceDisplayLabel(source, sourceIndex),
      audio,
    };
  }
  ...
  function showAudioSourceMenu(button, event) {
    ...
    const updateAnkiSelectionButtons = () => {
      ...
      menu.querySelectorAll(".audio-source-menu-export").forEach((exportButton) => {
        const sourceIndex = Number(exportButton.dataset.audioSourceIndex) || 0;
    ...
    const appendMenuItem = (source, sourceIndex, candidate, candidateIndex, label) => {
        ...
        candidates.forEach((candidate, candidateIndex) => {
          const numberedLabel =
            candidates.length > 1
              ...
              : sourceLabel;
          const candidateName = normalizeWhitespace(candidate && candidate.name);
          appendMenuItem(
            source,
            index,
            candidate,
            candidateIndex,
            candidateName ? numberedLabel + ": " + candidateName : numberedLabel,
          );
        });
        return;
      }
  ...
  function nodeHref(node) {
    ...
    return (
      node.href || node.url || data.href || data.url || attrs.href || attrs.url || ""
    );
  }
  ...
  function inheritedPopupThemeHint() {
    const configuredHint = String((state.config && state.config.popupThemeHint) || "")
      .trim()
      .toLowerCase();
  ...
  function ensurePopupThemeHintListener() {
    ...
    try {
      ...
      const handler = function () {
        if (normalizePopupTheme(state.config && state.config.popupTheme) === "inherit")
  ...
  function isLookupableChar(ch) {
    ...
    const policy =
      ...
      (lang.id === "ja" ? IINATAN_LOOKUP_CHARACTER_POLICY.policies.japanese : null);
  ...
  function lookupAnchorForUnit(unit, fallback) {
    if (state.config.experimentalNativeSubtitleHitLayer) return fallback || null;
    if (unit && unit.isWord) {
  ...
  function applyConfig(config) {
    ...
    state.config.nestedPopupMaxDepth = Math.max(
      1,
      Math.min(99999, Math.round(Number(state.config.nestedPopupMaxDepth) || 3)),
    );
  ...
  function nativeGeometryContextReason() {
    ...
    for (const element of [document.documentElement, document.body, overlayRoot]) {
      let computed;
      let rect;
      try {
  ...
  function ensureNativeSubtitleDom() {
    ...
    setImportantStyle(nativeSubtitleMatchHighlightsEl, "pointer-events", "none");
    ...
    setImportantStyle(nativeSubtitleHitBoxesEl, "writing-mode", "horizontal-tb");
  ...
  function nativeLayoutFontState(nativeLayout) {
    const options =
      nativeLayout && nativeLayout.options && typeof nativeLayout.options === "object"
  ...
  function publishNativeSubtitleDomPerformance(startedAt, mode, hitTargetCount) {
  ...
  function renderNativeAssAlphaMask(mask, geometry, opacity, copyElement) {
    ...
    if (!mask || mask.encoding !== "rle-u8-base64" || typeof mask.data !== "string")
      return false;
    let encoded;
    try {
      const decode =
  ...
  function nativeTextRangeRects(copyElement, start, end) {
    ...
    if (!nodes.length || !Number.isInteger(start) || !Number.isInteger(end)) return [];
        ...
        if (line.length) {
          if (
            IINATAN_NATIVE_SUBTITLE_HIT_LAYER.rectanglesSpanMultipleLines(
              nativeTextRangeRects(nativeSubtitleCopyEl, offset, offset + line.length),
            )
          )
            return true;
        }
  ...
  function nativeFontProbeSamples(text) {
    ...
    if (/[\uac00-\ud7af]/.test(cue)) return [cue || "한글", "가나다라마바사한글"];
  ...
  function releaseNativeSubtitleLocalFont() {
    ...
    if (!current || !document.fonts || typeof document.fonts.delete !== "function")
      return;
  ...
  function nativeSubtitleFallbackFaces(resolvedFace) {
    ...
    const seen = Object.create(null);
    runs.forEach((run) => {
      const name = String(run && run.postScriptName ? run.postScriptName : "").trim();
  ...
  function nativeSubtitleFontResolution(record, target, isCurrentCueGeneration) {
  ...
  function nativeSubtitleFontFamilyForMeasurement(
    ...
    fallbackFaces.forEach((face, index) => {
      faceRequests.push({
        name: face.name,
        alias: "iinatan-native-subtitle-fallback-font-" + generation + "-" + index,
        fallback: true,
    ...
    try {
      faceRequests.forEach((request) => {
        const source =
          'local("' + request.name.replace(/\\/g, "\\\\").replace(/"/g, '\\"') + '")';
        const face = new FontFaceConstructor(request.alias, source, {
          weight: resolvedFace && resolvedFace.resolvedBold ? "700" : "400",
          style: resolvedFace && resolvedFace.resolvedItalic ? "italic" : "normal",
        });
        faceRecords.push({ face, added: false });
        if (request.fallback) fallbackAliases[request.name] = '"' + request.alias + '"';
      });
    } catch (_) {
    ...
    const record = {
      ...
      fallbackAliases,
      fallbackRequired,
      primaryFontMetricScale: Number(resolvedFace && resolvedFace.fontMetricScale),
      localFontVerified: false,
      ready: null,
    };
  ...
  function renderNativeSubtitleFallbackRuns(
    ...
    const appendText = (valueToAppend) => {
      if (valueToAppend) target.appendChild(document.createTextNode(valueToAppend));
    };
  ...
  function applyNativeLayout(layout, balanceWrapSupported) {
    ...
    setImportantStyle(nativeSubtitleCopyEl, "max-width", layout.maxWidth + "px");
    ...
    setImportantStyle(nativeSubtitleCopyEl, "font-size", layout.fontSize + "px");
    setImportantStyle(nativeSubtitleCopyEl, "line-height", layout.lineHeight + "px");
    setImportantStyle(
      nativeSubtitleCopyEl,
      "letter-spacing",
    ...
    setImportantStyle(nativeSubtitleCopyEl, "-webkit-text-stroke", "0 transparent");
    ...
    setImportantStyle(nativeSubtitleCopyEl, "transform", layout.transform || "none");
    setImportantStyle(
      nativeSubtitleCopyEl,
      "top",
  ...
  function renderNativeSurfaceHitBoxes(measured, viewport) {
    const boxes = IINATAN_NATIVE_SUBTITLE_HIT_LAYER.resolveHitBoxOverlaps(measured, 2);
    boxes.forEach((box) => {
      if (
        box.left < -3 ||
        box.top < -3 ||
  ...
  function renderNativeSubtitleSurfaces(surfaces) {
    ...
    const list = Array.isArray(surfaces)
      ? surfaces.filter((surface) => surface && !surface.reason && surface.layout)
      : [];
    if (!list.length) {
    ...
    if (contextReason || !ensureNativeSubtitleDom()) {
      invalidateNativeSubtitleHitLayer(contextReason || "non-coextensive-overlay");
      return;
    }
    nativeSubtitleCopyEl.textContent = "";
    setImportantStyle(nativeSubtitleCopyEl, "all", "initial");
    ...
    const copyOpacity = Math.max(
      0,
      Math.min(1, Number(state.config.experimentalNativeSubtitleTextOpacity) || 0),
    );
    list.forEach((surface, surfaceIndex) => {
      ...
      if (Array.isArray(layout.directRects)) {
        ...
        if (copyOpacity > 0)
          renderNativeAssAlphaMask(layout.alphaMask, geometry, copyOpacity, copy);
      ...
      const calculated = IINATAN_NATIVE_SUBTITLE_HIT_LAYER.calculatePlainTextLayout(
        geometry,
        layout.options,
      );
      ...
      eventBlocks.forEach((block, blockIndex) => {
        ...
        copy.dataset.surface = role;
        if (eventBlocks.length > 1) copy.dataset.eventIndex = String(blockIndex);
        nativeSubtitleCopyEl.appendChild(copy);
            ...
            nativeSubtitleFontFamilyForMeasurement(
              ...
              .then((fontLoad) => {
                ...
                const fontSetAccepted =
                  localFontsVerified ||
                  fontSet.check(fontLoad.fontSpec, String(block.displayText || ""));
      ...
      Object.keys(groups).forEach((role) => {
        ...
        records.forEach((record) => {
          ...
          rowOffset +=
            Math.max(
              1,
              rows.length || String(record.block.displayText || "").split("\n").length,
    ...
    const finish = () => {
      ...
      plainRecords.forEach((record) => {
        ...
        const localLength = Array.from(String(record.block.lookupText || "")).length;
        for (
    ...
    const scheduleFinish = () => {
      if (!isCurrent()) return;
      stackEventBlocks();
      if (plainRecords.length && typeof window.requestAnimationFrame === "function")
  ...
  function renderNativeSubtitleHitLayer(
    ...
    state.nativeLookupSpans = Array.isArray(lookupSpans) ? lookupSpans.slice() : [];
    ...
    if (
      ...
      else overlayDebug("native subtitle hit layer skipped: text-index-map-failed");
    ...
    if (geometry.ok && Array.isArray(nativeLayout.directRects)) {
      ...
      if (nativeSubtitleHitBoxesEl.children.length) {
        setImportantStyle(nativeSubtitleHitBoxesEl, "display", "block");
        nativeSubtitleHitBoxesEl.classList.remove("hidden");
        publishAcceptedNativeLayoutDiagnostic(null, geometry, viewport, nativeLayout);
        publishNativeSubtitleDomPerformance(
          domUpdateStartedAt,
          "native-ass",
    ...
    const measure = () => {
      ...
      if (!balanceWrapSupported && nativeCopyRequiresAutomaticWrap(displayText)) {
      ...
      publishAcceptedNativeLayoutDiagnostic(layout, geometry, viewport, nativeLayout);
      nativeSubtitleHitBoxesEl.textContent = "";
      state.charByPos = Object.create(null);
      const measured = [];
      ...
      if (isCurrentCueGeneration() && nativeSubtitleHitBoxesEl.children.length) {
        setImportantStyle(nativeSubtitleHitBoxesEl, "display", "block");
        nativeSubtitleHitBoxesEl.classList.remove("hidden");
        publishNativeSubtitleDomPerformance(
          domUpdateStartedAt,
    ...
    if (
      ...
      Promise.resolve(fontSet.ready)
        ...
        .then((fontLoad) => {
          ...
          const localFontsVerified = fontLoad.fontResolution.localFontVerified === true;
          const fontSetAccepted =
          ...
          if (!fontSetAccepted || !fontFamilyAvailable || !fallbackRunsRendered) {
  ...
  function renderSubtitle(
    ...
    state.nativeSurfaces = Array.isArray(nativeSurfaces) ? nativeSurfaces.slice() : [];
  ...
  function groupMatchRects(rects) {
    ...
    rects.forEach((rect) => {
      ...
      let group = groups.find(
        (item) => Math.abs(item.top - rect.top) < Math.max(3, rect.height * 0.35),
      );
      if (!group) {
        group = {
          top: rect.top,
  ...
  function addNativeMatchBackgroundForRects(rects) {
    ...
    groupMatchRects(rects).forEach((group) => {
      ...
      setImportantStyle(bg, "width", Math.max(1, group.right - group.left + 10) + "px");
      setImportantStyle(bg, "height", Math.max(1, group.bottom - group.top + 6) + "px");
      ...
      setImportantStyle(bg, "box-shadow", "0 0 0 1px rgba(255,255,255,0.14) inset");
      nativeSubtitleMatchHighlightsEl.appendChild(bg);
    });
  }

  function activeMatchElements(start, len) {
    if (state.config.experimentalNativeSubtitleHitLayer && nativeSubtitleHitBoxesEl) {
      const end = start + len;
      return Array.from(nativeSubtitleHitBoxesEl.children || []).filter((el) => {
  ...
  function onCharEnter(ev) {
    if (state.config.subtitleLookupMode === "shift-hover" && !(ev && ev.shiftKey))
      return;
  ...
  document.addEventListener(
    ...
    (ev) => {
      ...
      if (popup && !popup.classList.contains("hidden")) trapPopupWheel(ev, popup);
    },
  ...
  function postPluginMessage(payload) {
    ...
    try {
      ...
      try {
        console.log("[iinatan overlay] plugin postMessage failed " + String(error));
      } catch (_) {}
      return false;
    }
  }
  ...
  function controllerPlaySelectedAudio() {
    return activateControllerControl(controllerSelectedControl(".audio-button"));
  }
  ...
  function setControllerAudioFocus(rowIndex, columnIndex) {
    ...
    rows.forEach((row) =>
      row
        ...
        .forEach((control) => control.classList.remove("controller-audio-focused")),
    );
  ...
  function controllerTargetRows(targets) {
    ...
    targets
      ...
      .forEach((target) => {
        ...
        row.centerY =
          row.targets.reduce((sum, item) => sum + item.centerY, 0) / row.targets.length;
      });
    rows.forEach((row) =>
  ...
  function navigateControllerSubtitle(direction) {
    ...
    if (direction === "left" || direction === "right") {
      const ordered = targets.slice().sort((left, right) => left.pos - right.pos);
  ...
  function scrollControllerPopupSmooth(axis, deltaMs) {
    ...
    const normalized =
      ((magnitude - CONTROLLER_AXIS_DEAD_ZONE) / (1 - CONTROLLER_AXIS_DEAD_ZONE)) *
      (axis < 0 ? -1 : 1);
    target.scrollTop =
  ...
  function handleHostControllerCommand(command, payload = {}) {
    switch (String(command || "")) {
      case "select-entry":
        return selectLargestVisibleControllerEntry();
      case "move-entry":
        return moveControllerEntry(
          String(payload.direction || "right") === "left" ? -1 : 1,
        );
      case "scroll-up":
      ...
      case "scroll-down":
        return scrollControllerPopupNotch(1);
      case "scroll-axis":
        return scrollControllerPopupSmooth(payload.axis, payload.deltaMs);
      case "close-audio-menu":
        hideAudioSourceMenu();
        return true;
      case "audio-up":
      ...
      case "close-popup":
        ...
        return true;
      case "hold-progress":
        if (payload.visible === false) hideControllerHoldProgress();
        else showControllerHoldProgress(payload.progress);
        return true;
      default:
        return false;
    }
  ...
  function controllerHoldActionAvailable(action) {
    ...
    if (action === "audio-menu")
      return controllerControlAvailable(controllerSelectedControl(".audio-button"));
  ...
  function startControllerHold(action, now) {
    if (state.controller.hold || !controllerHoldActionAvailable(action)) return false;
  ...
  function normalizeControllerGamepad(gamepad) {
    ...
    const pressed = (index) => {
      ...
      return !!(button && (button.pressed || Number(button.value || 0) >= 0.65));
    };
    const axis = (index) => {
  ...
  function dispatchControllerButton(button, now) {
    ...
    if (
      ...
      (action === "audioMenu" || action === "ankiPrimary" || action === "ankiForceAdd")
    )
        ...
        reportControllerStatus({
          ...
          buttonCount:
            selectedGamepad && selectedGamepad.buttons
              ...
              : 0,
          axisCount:
            selectedGamepad && selectedGamepad.axes ? selectedGamepad.axes.length : 0,
        });
      } catch (error) {
        reportControllerStatus({
          reason: "poll-error",
  ...
  function ankiPrimaryButtonForGroup(group) {
    ...
    if (group.classList && group.classList.contains("anki-button")) return group;
  ...
  function setNoteIdsForAnkiButton(control, noteIds) {
    ...
    group.dataset.ankiNoteIds = JSON.stringify(Array.isArray(noteIds) ? noteIds : []);
  }
  ...
  function noteIdsForButton(control) {
    ...
    try {
      const ids = JSON.parse(
        String(group && group.dataset ? group.dataset.ankiNoteIds || "[]" : "[]"),
      );
  ...
  function selectionIsInsidePopup(selection) {
    ...
    if (selection.rangeCount > 0 && typeof selection.getRangeAt === "function") {
      for (let i = 0; i < selection.rangeCount; i++) {
        ...
        if (!range || !popupContainsNode(range.commonAncestorContainer)) return false;
      }
      return true;
    }
    return false;
  ...
  function ankiPayloadContext(context, control) {
    ...
    const selected =
      popupSelectionText() ||
      normalizeWhitespace(
        group && group.dataset ? group.dataset.ankiPopupSelectionText || "" : "",
      );
    ...
    const audioReading = String(
      rawContext.audioReading ||
        rawContext.reading ||
        (rawContext.entry && rawContext.entry.term && rawContext.entry.term.reading) ||
        "",
    ).trim();
    ...
    return Object.assign({}, rawContext, {
      ...
      wordAudioSelection: audioSelection ? Object.assign({}, audioSelection) : null,
    });
  }
  function ankiGroupSnapshot(group) {
    return {
      state: String((group && group.dataset && group.dataset.ankiState) || "ready"),
      duplicate:
        !!group && !!group.dataset && group.dataset.ankiDuplicateKnown === "duplicate",
  ...
  function sendPendingAnkiWebSocket(pending) {
    ...
    const sent = sendBridgeMessage(ankiTransportPayload(pending.payload, "websocket"));
    if (!sent) return false;
    pending.transports.websocket = true;
    overlayDebug(
      "Anki request dispatched transport=websocket requestId=" + pending.requestId,
    );
    return true;
  }
  ...
  function schedulePendingAnkiAlternate(pending, preferredTransport) {
    ...
    const attempt = () => {
      if (!state.pendingAnkiMessages[pending.requestId] || pending.acked) return;
  ...
  function clearPendingAnkiStatusForButton(control) {
    const group = ankiActionGroupForControl(control);
    const key = group && group.dataset ? String(group.dataset.ankiContextId || "") : "";
  ...
  function trackAnkiBridgeMessage(group, payload, type, role, snapshot) {
    ...
    const finalDelay =
      type === "anki-card-status" ? 12000 : type === "anki-card-open" ? 30000 : 45000;
  ...
  function sendAnkiCardMessage(control, type) {
    ...
    if (group.dataset.ankiBusyRole) {
      setStatus({
        message:
          type === "anki-card-open" ? "Opening in Anki..." : "Adding Anki card...",
        kind: "info",
      });
      return true;
    }
    ...
    const payload = {
      ...
      noteIds: noteIdsForButton(group),
      duplicateKnown:
        group && group.dataset ? String(group.dataset.ankiDuplicateKnown || "") : "",
      ...
      popup.querySelectorAll(".anki-action-group").forEach((group) => {
        group.querySelectorAll(".anki-button").forEach((button) => {
          ...
          button.addEventListener("click", (event) => {
            ...
            const type =
              button.dataset.ankiAction === "open" ? "anki-card-open" : "anki-card-add";
  ...
  function placePopup(anchor) {
    ...
    let regionBottom = placeAbove ? rect.top - gap : window.innerHeight - margin;
    ...
    if (
      ![safetyTop, safetyBottom, safetyLeft, safetyRight].every(Number.isFinite) ||
    ...
    state.nestedPopups.forEach((item) => {
      ...
      try {
        ...
        highlights.forEach((highlight) => {
          if (highlight && typeof highlight.remove === "function") highlight.remove();
        });
  ...
  function updateNestedPopupScanningState() {
    ...
    if (!enabled) clearNestedPopups(0);
      ...
      state.nestedPopups.some((item) => item.depth > state.config.nestedPopupMaxDepth)
    )
  ...
  function placeNestedPopup(item) {
    ...
    element.style.maxHeight = String(Math.floor(maxRenderedHeight / scale)) + "px";
    ...
    if (!position) {
      ...
      clamped.sort((a, b) => a.anchorOverlap - b.anchorOverlap || a.index - b.index);
      position = clamped[0];
    }
  ...
  function nestedLookupAnchorRect(range, node, offset) {
    try {
      ...
      const anchorStart = start < value.length ? start : Math.max(0, value.length - 1);
      const end = Math.min(
        value.length,
        anchorStart + (value.codePointAt(anchorStart) > 0xffff ? 2 : 1),
      );
      const anchorRange =
        typeof document.createRange === "function" ? document.createRange() : range;
  ...
  function nestedLookupSourceFromEvent(popup, event) {
    ...
    let offset =
      range && (range.startOffset !== undefined ? range.startOffset : range.offset);
    ...
    return {
      ...
      key: String(popupDepth(popup)) + ":" + String(position) + ":" + lookupText,
    };
  }
  ...
  function createNestedPopupHighlights(rects, depth) {
    ...
    return groupMatchRects(rects || []).map((rect) => {
      ...
      highlight.style.width = String(Math.max(1, rect.right - rect.left)) + "px";
      highlight.style.height = String(Math.max(1, rect.bottom - rect.top)) + "px";
  ...
  function nestedLookupRangeRects(source, start, end) {
    ...
    (source && Array.isArray(source.segments) ? source.segments : []).forEach(
      (segment) => {
        ...
        const localStart = chars.slice(0, overlapStart - segment.start).join("").length;
        const localEnd = chars.slice(0, overlapEnd - segment.start).join("").length;
        try {
          const range = document.createRange();
  ...
  function updateNestedPopupHighlight(item, stored, fallbackPosition) {
    ...
    if (!item.element.classList.contains("lookup-pending")) placeNestedPopup(item);
  }
  function sendNestedLookupRequest(req) {
    ...
    const sent =
      sendBridgeMessage(payload) || (req.attempts >= 6 && postPluginMessage(payload));
  ...
  function requestNestedLookup(item, source) {
    const requestId =
      "nested-" + state.popupSessionId + "-" + String(++state.nestedLookupRequestSeq);
    const req = {
      requestId,
      popupId: item.id,
      lineId: state.lineId,
  ...
  function openNestedPopup(parentPopup, source) {
    ...
    const id =
      "nested-popup-" + String(depth) + "-" + String(state.nestedLookupRequestSeq + 1);
  ...
  function onNestedPopupClick(event) {
    if (
      ...
      (state.config.nestedPopupMode === "shift-hover" && !(event && event.shiftKey)) ||
      popupSelectionIsActive()
    )
      return;
    const popup = popupContainerForNode(event && event.currentTarget);
    if (!popup || nestedPopupClickIsInteractive(event && event.target, popup)) return;
    ...
    const child = state.nestedPopups.find(
      (item) => item.parentPopup === popup && item.depth === popupDepth(popup) + 1,
    );
  ...
  function bindPopupContainerEvents(popup) {
    if (!popup || (popup.dataset && popup.dataset.popupEventsBound === "true")) return;
    ...
    if (typeof node === "object") {
      ...
      if (node.tag === "ruby") {
        return toArray(node.content)
          .filter((part) => !(part && typeof part === "object" && part.tag === "rt"))
  ...
  function nodeDataContent(node) {
    ...
    return String(
      data.content || data["data-content"] || node.dataContent || node.kind || "",
    );
  }
  function nodeClassName(node) {
    const data = node && node.data ? node.data : {};
    const attrs =
      node && (node.attributes || node.attrs) ? node.attributes || node.attrs : {};
  ...
  function isGrammarDetails(node) {
    ...
    return (
      node && node.tag === "details" && /^grammar\b/i.test(detailsSummaryText(node))
    );
  }
  function isEtymologyDetails(node) {
    ...
    return (
      node && node.tag === "details" && /^etymology\b/i.test(detailsSummaryText(node))
    );
  }
  function detectDictionarySource(glossaryItem, parsed) {
    ...
    if (
      ...
      findNodes(parsed, (n) => isGrammarDetails(n) || isEtymologyDetails(n)).length
    )
  ...
  function etymologyShouldOpen(ctx) {
    ...
    const override = String(
      (state.config && state.config.wiktionaryEtymologyCollapseOverride) || "inherit",
    );
  ...
  function renderGrammarHtml(content, ctx) {
    ...
    if (!inline) return "";
    overlayDebug(
      "detected grammar section source=" + String((ctx && ctx.sourceKind) || "generic"),
    );
      ...
      rows
        .map(
          (row) =>
            ...
            (row.lemma
              ? '<span class="nonlemma-lemma">' + escapeHtml(row.lemma) + "</span>"
              : "") +
            (row.lemma && row.description
              ? '<span class="nonlemma-arrow"> - </span>'
              : "") +
            (row.description
              ? '<span class="nonlemma-desc">' + escapeHtml(row.description) + "</span>"
              : "") +
            "</span></div>",
        )
        .join("") +
  ...
  function structuredPosClass(text) {
    return isSelfFramedMarker(text) ? "pos-pill self-framed-inline-chip" : "pos-pill";
  }
  function renderStructuredSpan(node, ctx) {
    ...
    if (hasDataFlag(node, "headword") || /(?:見出|headword|カナ|かな|表記)/.test(cls))
    ...
    return (
      ...
      items
        .map((item) => {
          ...
          const content =
            item && typeof item === "object" && item.tag === "li" ? item.content : item;
          const body = renderStructuredNode(content, ctx);
          if (marker)
            return (
  ...
  function renderGlossaryLinesNode(node, ctx) {
    ...
    return (
      ...
      items.map((html) => '<div class="glossary-line">' + html + "</div>").join("") +
      "</div>"
    );
  }
  function formMarkerForCell(node) {
    const cls = nodeClassName(node);
    const title =
      nodeTitle(node) || findNodes(node, (n) => !!nodeTitle(n)).map(nodeTitle)[0] || "";
  ...
  function renderFormsNode(node, ctx) {
    return (
      '<div class="forms-block">' + renderStructuredNode(node.content, ctx) + "</div>"
    );
  }
  ...
  function renderStructuredNode(node, ctx) {
    ...
    if (kind === "sense-group")
      return (
        '<div class="sense-group">' + renderStructuredNode(node.content, ctx) + "</div>"
      );
    if (kind === "sense")
      return (
        '<div class="sense-body">' + renderStructuredNode(node.content, ctx) + "</div>"
      );
  ...
  function renderGlossaryPayload(glossaryItem) {
    ...
    if (!parsed) {
      return (
        metaRow +
        renderPlainGlossaryText((glossaryItem && glossaryItem.glossary) || "", ctx)
      );
    }
  ...
  function renderPitchPattern(reading, position) {
    ...
    if (!moras.length)
      return '<span class="pitch-number">[' + escapeHtml(String(pos)) + "]</span>";
    return (
  ...
  function renderEntryMetadata(term) {
    ...
    if (frequencyChips.length > 1) {
      const disclosureId = "freq-disclosure-" + String(++frequencyDisclosureSeq);
      ...
      chips.push(
        frequencyChips
          .slice(1)
          .map((html) =>
            html.replace('class="freq-chip"', 'class="freq-chip freq-chip-extra"'),
          )
          .join(""),
      );
  ...
  function buildAnkiCardContext(stored, entry, header, allowCurrentMedia) {
    ...
    const reading =
      (header && header.reading) || displayReadingForTerm(entry.term || {}, expression);
    ...
    return {
      lineId: state.lineId,
      position:
        stored && stored.position !== undefined ? stored.position : state.currentPos,
      sentence: useCurrentMedia ? state.text || "" : String(result.text || "").trim(),
      ...
      result: {
        ...
        language:
          result.language || (state.config && state.config.lookupLanguage) || "",
  ...
  function renderStoredLookupInto(popup, stored, isRootPopup) {
    ...
    if (!entries.length) {
      ...
      setPopupBodyFor(
        popup,
        '<div class="empty">No dictionary entry found from this ' + label + ".</div>",
      );
      return;
    }
    const first = entries[0];
    ...
    if (isRootPopup && state.currentPos !== null && state.currentPos !== undefined) {
    ...
    entries.slice(0, maxEntries).forEach((entry, entryIndex) => {
      const term = entry.term || {};
      html +=
        '<div class="entry" data-controller-entry-index="' + String(entryIndex) + '">';
  ...
  function setStatus(payload) {
    ...
    statusEl.className =
      payload.kind === "error" ? "error" : payload.kind === "success" ? "success" : "";
  ...
  function renderBitmapOcrStatus(payload) {
    ...
    if (status.state === "pending") {
      ...
      bitmapOcrStatusEl.setAttribute("aria-label", "Recognizing bitmap subtitle");
  ...
  iina.onMessage("controller-state", (payload) => {
    if (!payload || payload.protocol !== 1 || payload.source !== "native-hid") return;
    ...
    state.controller.nativeSnapshot = payload.connected
      ? {
          ...
          axes: Object.assign({ leftY: 0, rightX: 0, rightY: 0 }, payload.axes || {}),
        }
      : null;
    if (wasConnected !== !!state.controller.nativeSnapshot) resetControllerInput();
  ...
  iina.onMessage("native-layout-invalidate", (payload) => {
    ...
    invalidateNativeSubtitleHitLayer((payload && payload.reason) || "stale-layout");
  });
  ...
  iina.onMessage("subtitle", (payload) => {
    ...
    renderSubtitle(
      ...
      payload && Array.isArray(payload.nativeSurfaces) ? payload.nativeSurfaces : [],
  ...
  iina.onMessage("nested-lookup-result", (payload) => {
    ...
    const item = state.nestedPopups.find(
      (candidate) => candidate.id === req.popupId && candidate.requestId === requestId,
    );
    if (!item) return;
    const stored = Object.assign({}, payload, {
      position: payload.position === undefined ? req.position : payload.position,
    });
  ...
  window.__IINATAN_REFERENCE_POPUP__ = Object.freeze({
    state,
    elements: Object.freeze({
      root: document.getElementById("root"),
      popup: popupEl,
      nestedPopupLayer: nestedPopupLayerEl,
      popupSafetyZone: popupSafetyZoneEl,
      popupRowSafetyZone: popupRowSafetyZoneEl,
    }),
    applyConfig,
    clearNestedPopups,
    hideAudioSourceMenu,
    hidePopup,
    showAudioSourceMenu,
    rememberAudioCandidates,
    controllerOpenSelectedAudioMenu,
    handleHostControllerCommand,
    renderStoredLookupInto,
    setLookupPopupVisibility,
    setPopupBodyFor,
  });
})();

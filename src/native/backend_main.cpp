...
// release version. Package release metadata is owned by package.json.
static constexpr const char* WRAPPER_VERSION = "3.0.0";
...
static void cmd_worker(int argc, char** argv) {
  ...
  std::cerr << "iinatan-backend worker ready with " << cfg.dicts.size()
  ...
  while (!fs::exists(stop)) {
    if (owner_pid > 0 && std::chrono::steady_clock::now() >= next_owner_check) {
      if (!iinatan::platform::process_exists(owner_pid)) {
        std::cerr << "iinatan-backend worker stopping because owner pid " << owner_pid << " exited\n";
        break;
      }
  ...
  std::cerr << "iinatan-backend worker stopping\n";
}
...
static void cmd_version() {
  ...
  std::cout << "{\"ok\":true,\"name\":\"iinatan-backend\",\"backend\":\"Manhhao/hoshidicts\",\"wrapperVersion\":" << json_quote(WRAPPER_VERSION) << ",\"lookupProtocol\":1,\"geometryProtocol\":" << iinatan::ass::kAssGeometryProtocol << ",\"textLayoutProtocol\":" << iinatan::layout::kTextLayoutProtocol << ",\"target\":" << json_quote(std::string(target_os) + "-" + architecture) << ",\"architecture\":" << json_quote(architecture) << ",\"worker\":true,\"fontMetrics\":"
            ...
            << "},\"textLayout\":" << text_layout_service.capability().dump()
            << ",\"dependencies\":{\"harfbuzz\":" << json_quote(HB_VERSION_STRING)
            << ",\"freetype\":" << json_quote(std::to_string(FREETYPE_MAJOR) + "." + std::to_string(FREETYPE_MINOR) + "." + std::to_string(FREETYPE_PATCH))
            << ",\"fribidi\":" << json_quote(FRIBIDI_VERSION)
            << ",\"utf8proc\":" << json_quote(std::to_string(UTF8PROC_VERSION_MAJOR) + "." + std::to_string(UTF8PROC_VERSION_MINOR) + "." + std::to_string(UTF8PROC_VERSION_PATCH))
            << ",\"zlib\":" << json_quote(zlibVersion())
            << ",\"zstd\":" << json_quote(ZSTD_versionString())
            << "},\"platformAdapter\":" << json_quote(iinatan::platform::adapter_name())
            << ",\"http\":{\"available\":true,\"libcurl\":" << json_quote(iinatan::http::version()) << "},\"audioPreview\":{\"available\":true,\"ffmpeg\":" << json_quote(iinatan::audio::ffmpeg_version()) << ",\"output\":" << json_quote(iinatan::audio::output_version()) << "},\"bitmapOcr\":"
...
static const iinatan::protocol::Json& config_field(
    const iinatan::protocol::Json& object, const std::string& name) {
  const auto* value = object.find(name);
  if (!value) throw std::runtime_error("config is missing " + name);
  return *value;
}

static bool config_color(const iinatan::protocol::Json& value) {
  if (!value.is_string() || value.string().size() != 6) return false;
  return std::all_of(
      value.string().begin(), value.string().end(),
      [](unsigned char character) { return std::isxdigit(character) != 0; });
}

static void validate_config_v2(const iinatan::protocol::Json& root) {
  if (!root.is_object()) throw std::runtime_error("config must be an object");
  const auto& schema = config_field(root, "schemaVersion");
  if (!schema.is_number() || schema.integer() != 2)
    throw std::runtime_error("config schemaVersion must be 2");
  const auto& global = config_field(root, "global");
  if (!global.is_object()) throw std::runtime_error("config global must be an object");
  for (const char* name : {"backendPath", "ffmpegPath", "logPath"})
    if (!config_field(global, name).is_string())
      throw std::runtime_error(std::string("config global.") + name + " must be a string");
  if (const auto* recommendations = global.find("recommendedDictionaries")) {
    if (!recommendations->is_array())
      throw std::runtime_error("recommendedDictionaries must be an array");
    for (const auto& recommendation : recommendations->array()) {
      if (!recommendation.is_object() ||
          !config_field(recommendation, "title").is_string() ||
          !config_field(recommendation, "url").is_string() ||
          config_field(recommendation, "url").string().rfind("https://", 0) != 0)
        throw std::runtime_error("recommended dictionary entries require title and HTTPS URL");
    }
  }
  const auto& dictionaries = config_field(root, "dictionaries");
  const auto& pending = config_field(root, "pendingDictionaries");
  if (!dictionaries.is_array() || !pending.is_array())
    throw std::runtime_error("dictionary registries must be arrays");
  for (const auto& dictionary : dictionaries.array()) {
    if (!dictionary.is_object())
      throw std::runtime_error("dictionary entries must be objects");
    for (const char* name : {"id", "title", "path"})
      if (!config_field(dictionary, name).is_string() ||
          config_field(dictionary, name).string().empty())
        throw std::runtime_error(std::string("dictionary ") + name + " is invalid");
  }
  const auto& active = config_field(root, "activeProfileId");
  const auto& profiles = config_field(root, "profiles");
  if (!active.is_string() || active.string().empty() || !profiles.is_object() ||
      profiles.object().empty() || !profiles.find(active.string()))
    throw std::runtime_error("active profile is invalid");
  const std::array<std::string, 6> languages{"ja", "en", "de", "fr", "ko", "zh"};
  const std::array<std::string, 3> presets{"dark", "light", "high-contrast"};
  for (const auto& [id, profile] : profiles.object()) {
    if (id.empty() || !profile.is_object())
      throw std::runtime_error("profile entries must be named objects");
    if (!config_field(profile, "name").is_string() ||
        config_field(profile, "name").string().empty() ||
        !config_field(profile, "dictionaries").is_array() ||
        !config_field(profile, "audio").is_object() ||
        !config_field(profile, "anki").is_object())
      throw std::runtime_error("profile required fields are invalid");
    const auto& language = config_field(profile, "lookupLanguage");
    if (!language.is_string() ||
        std::find(languages.begin(), languages.end(), language.string()) == languages.end())
      throw std::runtime_error("profile lookupLanguage is invalid");
    const auto& theme = config_field(profile, "theme");
    if (!theme.is_object()) throw std::runtime_error("profile theme must be an object");
    const auto& preset = config_field(theme, "preset");
    if (!preset.is_string() ||
        std::find(presets.begin(), presets.end(), preset.string()) == presets.end())
      throw std::runtime_error("profile theme preset is invalid");
    for (const char* name : {"background", "foreground", "accent", "muted", "border"})
      if (!config_color(config_field(theme, name)))
        throw std::runtime_error(std::string("profile theme.") + name + " is invalid");
  }
}
...
static void cmd_fs_commit(int argc, char** argv) {
  ...
  validate_config_v2(iinatan::protocol::Json::parse(
      read_file_limited(target, 8 * 1024 * 1024)));
  copy_file_atomic(target, backup);
  std::cout << "{\"ok\":true}\n";
}
...
static bool safe_external_http_url(const std::string& url) {
  const bool https = url.rfind("https://", 0) == 0;
  const bool loopback =
      url.rfind("http://127.0.0.1", 0) == 0 ||
      url.rfind("http://localhost", 0) == 0 ||
      url.rfind("http://[::1]", 0) == 0;
  return (https || loopback) && url.size() <= 4096 &&
      url.find_first_of("\r\n") == std::string::npos;
}

static void cmd_open_url(int argc, char** argv) {
  if (argc != 3) throw std::runtime_error("usage: open-url <https_url>");
  const std::string url = argv[2];
  if (!safe_external_http_url(url))
    throw std::runtime_error("open-url rejected unsafe URL");
  std::error_code error;
  if (!iinatan::platform::open_external_url(url, error))
    throw std::runtime_error("could not open URL: " + error.message());
  std::cout << "{\"ok\":true}\n";
}
...
static void cmd_audio_preview(int argc, char** argv) {
  ...
  if (!safe_external_http_url(source_list))
    throw std::runtime_error("audio-preview rejected unsafe source URL");
  ...
  try {
    ...
    if (!safe_external_http_url(candidate))
      throw std::runtime_error("audio-preview rejected unsafe candidate URL");
...
int main(int argc, char** argv) {
  try {
    ...
    else if (command == "open-url") cmd_open_url(argc, argv);

...
int native_close(int fd) { return _close(fd); }
int native_read(int fd, void* buffer, unsigned int size) {
  return _read(fd, buffer, size);
}
int64_t native_seek(int fd, int64_t offset, int origin) {
  return static_cast<int64_t>(_lseeki64(fd, offset, origin));
}
bool native_fstat(int fd, NativeStat& status) {
  return _fstat64(fd, &status) == 0;
}
bool native_stat(const std::string& path, NativeStat& status) {
  const fs::path wide_path = fs::u8path(path);
  return _wstat64(wide_path.c_str(), &status) == 0;
}
int native_open(const std::string& path) {
  const fs::path wide_path = fs::u8path(path);
  return _wopen(wide_path.c_str(), _O_RDONLY | _O_BINARY);
}
bool native_regular(const NativeStat& status) {
  return (status.st_mode & _S_IFMT) == _S_IFREG;
}
std::string native_canonical_path(const std::string& path) {
  std::error_code error;
  const fs::path canonical = fs::weakly_canonical(fs::u8path(path), error);
  if (error) return {};
  const auto utf8 = canonical.u8string();
  return std::string(
      reinterpret_cast<const char*>(utf8.data()), utf8.size());
}
int64_t native_modified_ns(const NativeStat& status) {
  return static_cast<int64_t>(status.st_mtime) * 1'000'000'000;
}
#else
...
bool native_fstat(int fd, NativeStat& status) { return fstat(fd, &status) == 0; }
bool native_stat(const std::string& path, NativeStat& status) {
...
bool native_regular(const NativeStat& status) { return S_ISREG(status.st_mode); }
std::string native_canonical_path(const std::string& path) {
...
int64_t native_modified_ns(const NativeStat& status) {
  ...
  return static_cast<int64_t>(status.st_mtimespec.tv_sec) * 1'000'000'000 +
      status.st_mtimespec.tv_nsec;
#else
  return static_cast<int64_t>(status.st_mtim.tv_sec) * 1'000'000'000 +
      status.st_mtim.tv_nsec;
#endif
}
#endif

bool absolute_media_path(const std::string& path) {
...
int read_packet(void* opaque, uint8_t* buffer, int size) {
  ...
  while (true) {
    const int count = native_read(fd, buffer, static_cast<unsigned int>(size));
...
int64_t seek_file(void* opaque, int64_t offset, int whence) {
  ...
  if (whence == AVSEEK_SIZE) {
    NativeStat status {};
    return native_fstat(fd, status) ? status.st_size : AVERROR(errno);
  }
  const int base = whence & ~AVSEEK_FORCE;
  if (base != SEEK_SET && base != SEEK_CUR && base != SEEK_END)
    return AVERROR(EINVAL);
  const int64_t result = native_seek(fd, offset, base);
...
DemuxResult demux_ass_source(
  ...
  if (source.path.empty() ||
      (!absolute_media_path(source.path) && !network && !observed_ass) ||
  ...
  if (network) {
    ...
    canonical = native_canonical_path(source.path);
    if (canonical.empty())
      return failure("media-open-failed", std::strerror(errno));
    file.fd = native_open(canonical);
    if (file.fd < 0)
      return failure("media-open-failed", std::strerror(errno));
    if (!native_fstat(file.fd, status))
      return failure("media-stat-failed", std::strerror(errno));
    if (!native_regular(status))
  ...
  media.modified_ns = native_modified_ns(status);
  media.stream_index = source.ff_index;
  if (observed_network_ass &&
...
bool can_apply_ass_observation(
    const protocol::GeometrySourceRequest& source,
    const std::string& ass_extradata, const std::string& ass_full) {
  return !source.path.empty() && !absolute_media_path(source.path) &&
...
bool demuxed_source_unchanged(
  ...
  if (!absolute_media_path(source.path) || media.network_source) return false;
  const std::string resolved = native_canonical_path(source.path);
  if (resolved.empty() || media.canonical_path != resolved)
    return false;
  NativeStat status {};
  if (!native_stat(resolved, status) || !native_regular(status))
    return false;
  const int64_t modified_ns = native_modified_ns(status);
